...
Item {
    ...
    anchors.fill: parent
    antialiasing: true
    renderTarget: Canvas.FramebufferObject   // smoother in many setups
    ...
    // ---------- SLIDEOUT TIP (NO transparency, slide-only) ----------
    Item {
    id: tipWrap
    ...
    property int gap: 4
    property int hiddenOffset: -5

    anchors.leftMargin: root.hovered ? gap : (gap + hiddenOffset)

    clip: true
    width: root.hovered ? tipPill.implicitWidth : 0

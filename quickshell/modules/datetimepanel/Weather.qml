...
Rectangle {
    ...
    // Auto-set from sunrise/sunset (night starts 1 hour after sunset)
    property bool isNight: false

    // Sunrise/sunset (ms since epoch, local-time interpreted)
    property double sunriseMs: NaN
    property double sunsetMs: NaN

    // Supply explicit per-condition icon paths (day/night).
    ...
    property string greeting: "Happy " + Qt.formatDateTime(new Date(), "dddd")
    ...
    function refresh() {
        greeting = "Happy " + Qt.formatDateTime(new Date(), "dddd")
